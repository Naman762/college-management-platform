							...
							<section class="mt-3">
								...
								<div class="collapse show mt-2" id="collapseOne">
									<table class="w-100 table-elements table-two-tr"cellpadding="2">
										...
										<?php $roll_no=$_SESSION['roll_no'];
											$max_semester = "select max(semester) as semester from student_subjects where roll_no = '$roll_no'";
											$max_run = mysqli_query($con, $max_semester);
											$row = mysqli_fetch_array($max_run);
											$semester = $row['semester'];
											$query="select sc.subject_code,cs.subject_name,sc.semester,cs.credit_hours, sc.roll_no from student_subjects sc inner join course_subjects cs on sc.subject_code = cs.subject_code where sc.roll_no='$roll_no' and sc.semester = '$semester'";
											$run=mysqli_query($con,$query);
											while ($row=mysqli_fetch_array($run)) { ?>								
										<tr>
								...
								<div class="collapse show mt-2" id="collapsethree">
									<table class="w-100 table-elements table-five-tr"cellpadding="2">
										...
										<?php 
											...
											$query="select * from student_subjects inner join course_subjects on student_subjects.subject_code=course_subjects.subject_code where student_subjects.roll_no='$roll_no' and student_subjects.semester = '$semester'";
											$run=mysqli_query($con,$query);
											while ($row=mysqli_fetch_array($run)) { ?>								
											<tr>
												<td><?php echo $row['subject_code'] ?></td>
